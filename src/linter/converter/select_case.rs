...
pub fn on_select_case(
    a: SelectCaseNode,
) -> impl Stateful<Output = SelectCaseNode, Error = QErrorNode, State = Context> {
...
fn on_case_block(
    a: CaseBlockNode,
) -> impl Stateful<Output = CaseBlockNode, Error = QErrorNode, State = Context> {
    let expression_list = Unit::new(a.expression_list).vec_flat_map(CaseExpressionStateful);
