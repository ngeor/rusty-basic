...
use crate::pc::{any_token, And, Filter, Parser, RcStringView, ToOption, Token};
use crate::pc_specific::TokenType;

/// Equal sign, surrounded by optional whitespace.
///
/// `<ws>? = <ws>?`
pub fn equal_sign() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `<ws>? , <ws>?`
pub fn comma() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `<ws>? * <ws>?`
pub fn star() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `:`
pub fn colon() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `<ws>? : <ws>?`
pub fn colon_ws() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `-`
pub fn minus_sign() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `.`
pub fn dot() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `#`
pub fn pound() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `$`
pub fn dollar_sign() -> impl Parser<RcStringView, Output = Token> {
...
///
/// `<ws>? ; <ws>?`
pub fn semicolon() -> impl Parser<RcStringView, Output = Token> {
    any_token_of_ws(TokenType::Semicolon)
}

/// Whitespace.
pub fn whitespace() -> impl Parser<RcStringView, Output = Token> {
...
pub fn digits() -> impl Parser<RcStringView, Output = Token> {
    any_token_of(TokenType::Digits)
}

pub fn any_token_of(token_type: TokenType) -> impl Parser<RcStringView, Output = Token> {
    any_token()
        .filter(move |token| token_type.matches(token))
        .with_expected_message(move || token_type.to_error_message())
}

fn any_token_of_ws(token_type: TokenType) -> impl Parser<RcStringView, Output = Token> {
