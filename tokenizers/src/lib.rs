...
use std::io::BufRead;
use std::iter;
use readers::{CharReader, create_reader};

use recognizers::{Recognition, Recognizer};
...
pub fn create_tokenizer<T: BufRead>(
    buf_read: T,
    recognizers: Vec<Box<dyn Recognizer>>,
) -> impl Tokenizer {
    UndoTokenizerImpl::new(TokenizerImpl::new(create_reader(buf_read), recognizers))
}
...
mod tests {
    ...
    use readers::{CharReader, create_reader};
    use recognizers::{many_digits_recognizer, many_letters_recognizer};
    use crate::{Tokenizer, TokenizerImpl, UndoTokenizerImpl};

    pub fn string_char_reader<T>(input: T) -> impl CharReader
        where
            T: AsRef<[u8]>,
    {
        create_reader(BufReader::new(Cursor::new(input)))
    }
