...
use std::str::Chars;
use recognizers::{any_single_char_recognizer, is_digit, is_letter, keyword_recognizer, leading_remaining_recognizer, many_digits_recognizer, many_white_space_recognizer, Recognition, Recognizer, single_char_recognizer, single_new_line_recognizer, str_recognizer};
use tokenizers::{create_tokenizer, Token, Tokenizer};
...
pub fn create_file_tokenizer(input: File) -> impl Tokenizer {
    create_tokenizer(BufReader::new(input), create_recognizers())
}

#[cfg(test)]
pub mod string_helper {
    use std::io::{BufReader, Cursor};
    use tokenizers::{create_tokenizer, Tokenizer};
    use crate::parser::specific::create_recognizers;

    pub fn create_string_tokenizer<T>(input: T) -> impl Tokenizer
        where
            T: AsRef<[u8]>,
    {
        create_tokenizer(BufReader::new(Cursor::new(input)), create_recognizers())
    }
